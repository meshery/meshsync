...
func New(log logger.Handler, informer dynamicinformer.DynamicSharedInformerFactory, broker broker.Handler, fileWriter io.Writer, plConfigs map[string]internalconfig.PipelineConfigs, stopChan chan struct{}) *pipeline.Pipeline {
	...
	for _, config := range configs {
		gdstage.AddStep(newRegisterInformerStep(log, informer, config, broker, fileWriter)) // Register the informers for different resources
	}
	...
	for _, config := range configs {
		ldstage.AddStep(newRegisterInformerStep(log, informer, config, broker, fileWriter)) // Register the informers for different resources
	}
