...
- **Testing Infrastructure**: `make test`, `make coverage-report`, integration tests via `make integration-tests`
- **Version Control**: Git with DCO (Developer Certificate of Origin) sign-off
- **Local Run**: NATS dependency (see `make nats-run` for local NATS server)
...
5. **Testing and Validation**: Ability to write unit tests and integration tests, verify eventual consistency, and debug event processing pipelines
   ...
   - Coverage via: `make coverage-report`
...
6. **Build System Proficiency**: Using MeshSync's Makefile for local builds, testing, linting, and Docker operations:
   ...
   - `make run` - Run local instance with NATS (starts NATS server via `make nats-run`)
   - `make lint-run` - Lint check using golangci-lint
   - `make test` - Run unit tests with race detection
   - `make coverage-report` - Generate coverage reports
   - `make docker-build` - Build Docker image `meshery/meshery-meshsync`
...
- Local development requires NATS server; use `make nats-run` to start it before `make run`
...
- Coverage should be verified: `make coverage-report`
...
- **Coverage**: Generate and inspect coverage reports with `make coverage-report`.​
...
- Run locally with NATS running: `make nats-run` then `make run`
...
### Build Commands
```bash
make nats-run                 # Start local NATS server
make build                # Build MeshSync binary to bin/meshsync
...
make lint-run                # Lint check with golangci-lint
```
### Important URLs
